...
def test_img_poison(net, datatest):
    ...
    if f.dataset == "gtsrb":
        ...
        correct  = torch.tensor([0.0] * 43)
        correct_pos = torch.tensor([0.0] * 43)
        correct_train = torch.tensor([0.0] * 43)
        # number of picture
        gold_all = torch.tensor([0.0] * 43)
        gold_all_pos = torch.tensor([0.0] * 43)
        gold_all_train = torch.tensor([0.0] * 43)
    else:
